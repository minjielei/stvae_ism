...
class ScatCovDataset():
    """
    A lass to load data from a HDF5 file containing scattering covariance data.

    Args:
        file_path (str): Path to the HDF5 file containing the scattering covariance data.
    ...
    def __init__(
        ...
        # Get the number of windows (images) in the scattering covariance dataset.
        self.num_windows = self.file['scat_cov'][
            self.scatcov_datasets[0]].shape[0]

        # Create a dictionary that stores the shape of the scat_cov datasets.
        self.shape = {
            'scat_cov': {}
        }
        ...
        # If normalize_data is True, compute the normalization parameters for
        # each dataset.
