...
def sepdir(path: str, mkdir: Optional[bool] = True) -> str:
    """The absolute path to the image separation logs directory.
    ...
    Args:
        ...
        mkdir: An optional boolean for whether to create the directory if it
            does not exist.
    """
    path = os.path.join(datadir('separation'), path)
    if (not os.path.exists(path)) and mkdir:
        os.makedirs(path)
    return path

def postdir(path: str, mkdir: Optional[bool] = True) -> str:
    """The absolute path to the post directory.

    Post directory stores postprocessed data. Here the path is created if it
    does not exist upon call if `mkdir` is True.
    ...
    Args:
        ...
        mkdir: An optional boolean for whether to create the directory if it
            does not exist.
    """
    path = os.path.join(datadir('postprocess'), path)
    if (not os.path.exists(path)) and mkdir:
        os.makedirs(path)
    return path
